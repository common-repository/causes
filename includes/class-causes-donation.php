...
function algo_causes_save_settings(){
	...
	if(isset($_GET['tab']) && ($_GET['tab']=='paypal' || $_GET['tab']=='donation' || $_GET['tab']=='mail' || $_GET['tab']=='causes-settings' || $_GET['tab'] =='stripe' || $_GET['tab']=='postEditor')){		
		
		foreach($_POST['causes_settings'] as $key=>$settings){
			
			/*if($key=='paypal_is_active'):
				update_option( 'stripe_is_active', '' );
			endif;
			
			if($key=='stripe_is_active'):
				update_option( 'paypal_is_active', '' );
			endif;*/
...
function algo_causes_checkout(){ 
  ...
  global $wpdb;
	...
	/// check if paypal payment method is active
	if($_POST['payment_method']=='paypal')	{ 
			...
			require_once plugin_dir_path(__DIR__) .'includes/express-checkout.php';
							...
							$dta = '<form role="form" method="POST" class="Modal-form" id="stripe-form"><script
									src="https://checkout.stripe.com/checkout.js" class="stripe-button" id="stripe-button"
									data-key="'.get_option('stripe_api_publishable_key').'"
									data-amount="'.number_format($total_donation*100,'2','.','').'"
									...
									data-bitcoin="false"
									data-panel-label="Donate Now"
									data-label="Pay With Stripe">
							 </script>
							 <input type="hidden" name="amount" value="'.number_format($total_donation*100,'2','.','').'">
							 <input type="hidden" name="order_id" value="'.$donatorId.'">
							 </form>';
							 
							 
							 
							 echo json_encode(array('loggedin'=>'stripe','message'=>$dta));
	...
	global $wpdb;	
		...
		if(isset($_REQUEST['token']) && $_REQUEST['token']!=''):			
			require_once plugin_dir_path(__DIR__) .'/includes/express-checkout.php';
			 ...
			 if(get_option('stripe_is_active')=='1'){
				require_once plugin_dir_path(__DIR__) .'/includes/class-causes-stripe-payment.php';
			 }
			
			if(isset($_POST['stripeToken']))
			{				
				Causes_Stripe_Payment::strip_checkout_response();
			}
                            ...
                            <tfoot>
                                ...
                                <tr>
                                    ...
                                    <td class="cart-total-row text-right">
                                        ...
                                        <span class="cart-total text-primary"><?php if($currencyCode=='before'){ do_action('causes_currency_code');  echo number_format($total,'2','.',''); } else {  echo number_format($total,'2','.',''); do_action('causes_currency_code');  } ?></span>
                                    </td>
                                </tr>
                            </tfoot>
                                ...
                                <div class="row">								
									...
									<div class="form-group">
                                            <div class="input-group radio">
                                               ...
                                               <div class="radio">
											   <input name="payment_method" type="radio" value="paypal" id="high" checked >
												<label for="high"><strong>Paypal Express</strong></label>
												</div>
                                            </div>
                                        </div>
										<?php  if(get_option('stripe_is_active')=='1'){ ?>
                                        <div class="form-group">
                                            <div class="input-group ">
                                                <span class="input-group-addon"><i class="fa fa-cc-stripe"></i></span>
												<div class="radio">
                                                <input name="payment_method" type="radio" value="stripe" id="low" >
												<label for="low"><strong>Crdit Card (Stripe) </strong></label>
												</div>
                                            </div>
                                        </div>
										<?php } ?>
...
jQuery('#confirm').on('shown.bs.modal', function (event){ jQuery("body").addClass('modal-open');  }); 
			...
			jQuery(".stripe-button-el").bind('click',function(){
			...
			jQuery('#personal-info').on("submit", function( e ) {
            	...
            	jQuery.post(causesLocalize.ajaxurl, jQuery("#personal-info").serialize() + '&action=algo_causes_checkout', function(response) {
				 ...
				 if(response.loggedin == false){
					...
					else if(response.loggedin =='stripe')
					{
							jQuery('.personal-info').html(response.message);
